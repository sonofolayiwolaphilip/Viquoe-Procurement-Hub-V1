...
import { useAuth } from "@/components/auth-provider" // ✅ CHANGE 1: Import auth hook
...
export default function RegisterPage() {
  // ✅ CHANGE 2: Get authentication state
  ...
  // ✅ CHANGE 3: Redirect logged-in users to their dashboard
  useEffect(() => {
    ...
    if (!authLoading && isAuthenticated && user) {
      ...
      if (user.userType === "buyer") {
        ...
        // Fallback to home if userType is unknown
        router.push("/")
      }
    }
      ...
      const { data, error: signUpError } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            ...
            businessRegistration: formData.businessRegistration.trim().toUpperCase(),
  ...
  // ✅ CHANGE 4: Show loading state while checking auth
  if (authLoading) {
    return <div className="min-h-screen flex items-center justify-center">Loading...</div>
  }

  // ✅ CHANGE 5: Return null while redirecting authenticated users
  // This prevents a flash of the registration form
  if (isAuthenticated) {
    return null
  }
